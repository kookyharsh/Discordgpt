...
class Guild(Base):
    ...
    settings: Mapped[Optional["GuildSettings"]] = relationship("GuildSettings", back_populates="guild", uselist=False, cascade="all, delete-orphan")
    conversations: Mapped[list["Conversation"]] = relationship("Conversation", back_populates="guild", cascade="all, delete-orphan")
    actionExecutions: Mapped[list["ActionExecution"]] = relationship("ActionExecution", back_populates="guild", cascade="all, delete-orphan")
    scheduledActions: Mapped[list["ScheduledAction"]] = relationship("ScheduledAction", back_populates="guild", cascade="all, delete-orphan")
    generatedCommands: Mapped[list["GeneratedCommand"]] = relationship("GeneratedCommand", back_populates="guild", cascade="all, delete-orphan")
    auditLogs: Mapped[list["AuditLog"]] = relationship("AuditLog", back_populates="guild", cascade="all, delete-orphan")
    confirmations: Mapped[list["ConfirmationRequest"]] = relationship("ConfirmationRequest", back_populates="guild", cascade="all, delete-orphan")

class GuildSettings(Base):
    __tablename__ = "GuildSettings"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    guildId: Mapped[str] = mapped_column(String, ForeignKey("Guild.id", ondelete="CASCADE"), unique=True, nullable=False)
...
class Conversation(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    guildId: Mapped[str] = mapped_column(String, ForeignKey("Guild.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        Index("Conversation_guildId_userId_idx", "guildId", "userId"),
    )
...
class ConversationMessage(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    conversationId: Mapped[str] = mapped_column(String, ForeignKey("Conversation.id", ondelete="CASCADE"), nullable=False)
...
class ActionExecution(Base):
    ...
    steps: Mapped[list["ActionExecutionStep"]] = relationship("ActionExecutionStep", back_populates="execution", cascade="all, delete-orphan")
    auditLogs: Mapped[list["AuditLog"]] = relationship("AuditLog", back_populates="execution")

    __table_args__ = (
        Index("ActionExecution_guildId_userId_idx", "guildId", "userId"),
    )
...
class ActionExecutionStep(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    executionId: Mapped[str] = mapped_column(String, ForeignKey("ActionExecution.id", ondelete="CASCADE"), nullable=False)
...
class ScheduledAction(Base):
    ...
    runs: Mapped[list["ScheduledActionRun"]] = relationship("ScheduledActionRun", back_populates="scheduledAction", cascade="all, delete-orphan")

    __table_args__ = (
        Index("ScheduledAction_guildId_enabled_idx", "guildId", "enabled"),
    )
...
class ScheduledActionRun(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    scheduledActionId: Mapped[str] = mapped_column(String, ForeignKey("ScheduledAction.id", ondelete="CASCADE"), nullable=False)
    status: Mapped[str] = mapped_column(String, nullable=False)
    result: Mapped[Any | None] = mapped_column(JSONB, nullable=True)
    error: Mapped[str | None] = mapped_column(Text, nullable=True)
    executedAt: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)

    scheduledAction: Mapped["ScheduledAction"] = relationship("ScheduledAction", back_populates="runs")
...
class AuditLog(Base):
    ...
    guildId: Mapped[str] = mapped_column(String, ForeignKey("Guild.id", ondelete="CASCADE"), nullable=False)
    userId: Mapped[str] = mapped_column(String, nullable=False)
    executionId: Mapped[str | None] = mapped_column(String, ForeignKey("ActionExecution.id", ondelete="SET NULL"), nullable=True)
    ...
    execution: Mapped[Optional["ActionExecution"]] = relationship("ActionExecution", back_populates="auditLogs")

    __table_args__ = (
        Index("AuditLog_guildId_createdAt_idx", "guildId", "createdAt"),
    )
...
class ConfirmationRequest(Base):
    ...
    __table_args__ = (
        Index("ConfirmationRequest_guildId_userId_actionNonce_idx", "guildId", "userId", "actionNonce"),
    )
